...
class Bank:
    ...
    def __init__(self, balance: List[int]):
      self.accounts = {account : money for account, money in enumerate(balance)}
      
    def transfer(self, account1: int, account2: int, money: int) -> bool:
      if account1 - 1 in self.accounts and self.accounts[account1 - 1] >= money and account2 - 1 in self.accounts:
    ...
    def withdraw(self, account: int, money: int) -> bool:
      if account - 1 in self.accounts and self.accounts[account - 1] >= money:
        self.accounts[account - 1] -= money
        return True
      else:
        return False
