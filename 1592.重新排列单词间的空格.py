...
class Solution:
    def reorderSpaces(self, text: str) -> str:
        ...
        if times == 0:
            return text
        if len(words) > 1:
            space_count = times // (len(words) - 1)
        else:
            space_count = 0
        # return [space_count, times, len(words)]
        new_text = ''
        space = [' ' * space_count]
        ...
        new_text += ''.join(words[-1])
        if space_count * (len(words) - 1) < times:
            new_text += ''.join([' ' * (times - space_count * (len(words) - 1))])
        return new_text'''
        ...
        times = text.count(' ')
        if len(words) == 1:
            return ''.join(words[0]) + ' ' * times
        len_space, res_space = divmod(times, len(words) - 1)
        return (' ' * len_space).join(words) + ' ' * res_space
